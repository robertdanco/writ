...
### Option D: Brownfield with codebase awareness

```
/writ-profile
# Claude profiles the codebase: files, imports, exports, test mappings

/writ-spec "add CSV export for activity log"
# Claude co-creates a change-level spec with you, using the profile for context

/writ-diff csv-export
# Preview which files and tests will be affected

/writ-session csv-export
# Implement the feature with codebase-aware planning
```
...
Four capabilities extend the base verification loop:

**Selective test execution.** If `/writ-profile` has generated `.writ/test-map.json`,
`/writ-verify` consults it for `test_passes` criteria during single-feature verification.
It finds which source files changed, looks up which tests cover them, and runs only those
tests. If selective tests fail, the full suite also runs as a safety net. Regression checks
(`--all`) always run the full suite.

**Criterion timeout.** `command_succeeds` and `test_passes` criteria are wrapped in
`timeout` (or `gtimeout` on macOS) to prevent hangs. Default: 60 seconds. Override
per-criterion with a `timeout` field, or project-wide via `defaults.criterion_timeout`
in writ.json.
...
**Smart criterion suggestions.** After all criteria pass, a read-only subagent analyzes
the git diff and feature spec to propose 0-3 additional criteria. Suggestions are written
to `.writ/runs/<feature-id>/suggestions.json` and can be applied via
`/writ-spec --amend <feature-id>`. Use `--no-suggestions` to skip, or `--auto` to log
...
```json
{
  ...
  "acceptance_criteria": [
    ...
    { "description": "Auth tests pass", "type": "test_passes",
      "target": "npm test -- --grep 'auth'", "expected": "", "timeout": 120 }
  ]
}
```

The optional `timeout` field on criteria overrides the default 60-second timeout for
that criterion. Set a project-wide default in the top-level `defaults` object:

```json
{
  "defaults": { "criterion_timeout": 90 },
  "features": [...]
}
```
...
mean more chances for the model to lose focus or skip a step. Writ has 10 commands
...
| `/writ-profile` | Generate codebase profile and test map. Extracts imports/exports via regex, builds dependency graph and source-to-test mappings. Output: `.writ/profile.json`, `.writ/test-map.json`. |
| `/writ-spec "description"` | Interactive spec co-creation with codebase awareness. Co-creates change-level features through structured conversation rounds. Loads profile for context. Emits writ.json delta with advisory scope. |
| `/writ-diff <feature-id>` | Pre-implementation impact preview. Shows affected files and tests from profile + test-map. Read-only. |
...
| `/writ-verify <feature-id>` or `--all` | Evaluate criteria with selective test execution, criterion timeouts (60s default), replay logging to `.writ/runs/`, and smart criterion suggestions. `--all` for regressions, `--pending` for discriminative checks. Flags: `--auto`, `--no-suggestions`. |
...
```
your-project/
├── .writ/                       # Codebase intelligence (created by init.sh)
│   ├── profile.json              # Generated by /writ-profile (gitignored)
│   ├── test-map.json             # Generated by /writ-profile (gitignored)
│   ├── spec-session.json         # /writ-spec resume state (gitignored)
│   └── runs/                     # Verification logs (gitignored)
│       └── <feature-id>/         # Per-feature run history
│           ├── <timestamp>-<hex>.json  # Replay log per verification run
│           └── suggestions.json  # Smart criterion suggestions (latest)
...
└── .claude/
    ├── commands/
    │   ├── writ-profile.md       # /writ-profile - Codebase profiling
    │   ├── writ-spec.md          # /writ-spec - Interactive spec co-creation
    │   ├── writ-diff.md          # /writ-diff - Pre-implementation impact preview
    ...
    │   ├── writ-verify.md        # /writ-verify - Enhanced verification with selective tests, timeouts, replay, suggestions
