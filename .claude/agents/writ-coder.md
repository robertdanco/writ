...
Your job is to implement exactly one feature from writ.json, verify it passes all
...
1. Run `cat writ.json` to load the full specification
...
1. Any feature with status `in_progress` in writ.json (resume interrupted work)
...
Update writ.json: set this feature's status to `"in_progress"`.
...
NEVER dump test output into your response. Log to `/tmp/writ-verify.txt`, print summary only.
...
Include this option:
- Revert to pre-session state (`git reset --hard writ-pre-<feature-id>`)
...
Update `writ.json`: set this feature's `status` to `"completed"`.
...
Run /writ-session (or spawn writ-coder) to continue.
```

## Hard constraints

- Implement ONLY the feature selected at session start
- NEVER modify writ.json except to update `status` fields
